...
; not divisible by 2, 3, 5. called fizzbuzz because it's the classic fizz-buzz example!
(define (fizz-buzz x)
  (if (and (not (even? x)) (not (divisible? x 3)) (not (divisible? x 5)))
      #t
      #f))

; gets all odd, and integers not divisible by  integers
; (stream-for-each (print-stream (stream-filter fizz-buzz integers)))
...
; defined because it's easier to read/write in a long stream declartion,
; like the one here
(define (not-divisble-by-3? x)
  (if (not (divisible? x 3))
      #t
      #f))

(define (not-divisble-by-7? x)
  (if (not (divisible? x 7))
      #t
      #f))

; interleaves integers not divisible by 3 or 7
; (print-stream (interleave (stream-filter not-divisble-by-3? integers)
;                          (stream-filter not-divisble-by-7? integers)))
...
(define alt2 (cons-stream 0 (merge integers alt)))
