  ...
  (let ((binding-count 0)
    ...
    (define (me message)
      (cond ((eq? message 'binding-count) binding-count)
            ...
            ((eq? message 'frame-count) frame-count)
            
            ((eq? message 'increase-frame-count)
             (set! frame-count (+ 1 frame-count)))
             
            ((eq? message 'increase-binding-count)
             (set! binding-count (+ 1 binding-count)))
